---
title: "Elegance! Hina-matsuri Rebellion: Card Analysis"
description: "Analysis of the new event cards from Elegance! Hina-matsuri Rebellion, including the Odairi and Ohina combo, new Divina supports, and a tanky Anima melee"
date: "2026-02-18"
author: "Araiak"
tags: ["analysis", "gacha", "events"]
---

<div data-lang="en">

After all the auction madness, here's the breakdown on the new event cards.

---

## Pre-Event Gacha

:card[1584] is an XP and drop rate boosting healer now in the standard pool. I don't think she displaces :card[1321] in most content - her skill is worse, and Divina doesn't have that many damaging spells to boost. She's usable if you find yourself with 5 copies, but probably not worth orbing. **Skip.**

---

## Event Cards

:card[1526] *(Limited Gacha)* and :card[1529] *(Overall Ranking)* are the pair to watch. Together they give :card[1526] +60% crit rate, 65% crit damage (progressive), and 60% final wave damage. I don't know yet if this will crack into the world boss meta - it's possible but we may have to wait and see. In any case the combo is very good across all content. :card[1526] probably already dethrones :card[1401] as the highest single-round damage card, though :card[1401] doesn't have the conditional requiring two card slots. Note that only :card[1529] grants the bonus to :card[1526] - :card[228] does not apply. These two will be the main targets for most players this event. **High priority pair - the best thing this event has to offer.**

:card[1528] *(Ranking Reward - 1st & 2nd Half)* is a fairly strong support or reserve pick for Divina teams. Her skill isn't very impactful, but her passives are solid - 25% attack speed for all Divina allies and 35% final wave damage for Divina team members. **Good Divina support, weak skill.**

:card[1531] *(March Exchange)* is a small upgrade to :card[715], increasing the effect by 2%. That's not enough to make the card especially relevant, but it's probably usable in some team comps if it's easily accessible. **Niche - pick up if easy to get.**

:card[1527] *(Standard Gacha)* is a 4★ but punches well above its rarity - 44% Crit DMG to the 2 allies with the highest ATK is a lot for a 4★, comparable to :card[600]. A strong reserve option for newer players who don't have better crit damage supports yet. **Strong reserve for newer players - grab if accessible.**

:card[1539] *(Special Story Reward)* is an extremely tanky Anima melee. 40% damage reduction is more than most content requires, but if you run into any situation where you need a rock-solid tank, this is one of the best options available. When her HP drops to 10% or below, she gains a permanent 50% damage boost and heals 10% HP over 5 seconds - so the lower she gets, the harder she hits. **Strong niche pick for any content where survivability is the bottleneck.**

:card[1540] *(Limited Gacha)* is a 10% progressive crit rate alternative to :card[1494]. He's probably better in Endless or reserve, while :card[1494] still edges out in XP modes thanks to the XP boost. If you don't have :card[1494] at level 75, this is one to target. **Good Divina reserve option, especially for Endless.**

:card[1590] *(Standard Gacha - post event)* is a decent defensive support that boosts damage for the top two Divina allies, while also providing major damage reduction on the final wave. She's comparable to :card[672] in reserve, but more defensive - :card[672] is the higher damage option for Divina reserve slots. **Solid Divina reserve for survivability-focused builds; :card[672] if you want more damage.**

---

## TL;DR

- :card[1526] (Limited Gacha) + :card[1529] (Overall Ranking) - **High priority pair** - top single-round Anima damage with the combo active, potential WB meta
- :card[1528] (Ranking - 1st & 2nd Half) - **Good Divina support** - solid passive, weak skill
- :card[1540] (Limited Gacha) - **Good Divina reserve** - better than :card[1494] in Endless
- :card[1539] (Special Story Reward) - **Strong Anima tank** - 40% DR if you need survivability
- :card[1590] (Standard Gacha - post event) - **Solid Divina reserve** - defensive support for WB and Special Story/Conquest
- :card[1527] (Standard Gacha) - **Strong reserve for newer players** - 44% Crit DMG to top 2 ATK allies, comparable to :card[600]
- :card[1531] (March Exchange) - **Niche** - small upgrade to :card[715], grab if accessible
- :card[1584] (Standard Gacha) - **Skip** - XP/drop healer, doesn't displace :card[1321]

---

*A note on recommendations: Play how you like! If you want a card, go for it. My recommendations highlight cards I see as standouts - ones unlikely to be replaced for some time - while being mindful of cost. The game is fairly generous, so you can pick up cards that aren't super strong and still have resources for the next event. Don't hesitate to buy a card you like!*

</div>

<div data-lang="ja">

オークションの騒ぎが落ち着いたところで、新イベントカードの解説です。

---

## イベント前ガチャ

:card[1584]はEXPとドロップ率をブーストするヒーラーで、スタンダードプール入りしました。ほとんどのコンテンツで:card[1321]を置き換えるとは思えません——スキルが劣り、Divinaにはそれほど多くのダメージスペルもないため活かしにくい。5枚揃ってしまった場合は使えますが、オーブを使って引く価値はないでしょう。**スキップ。**

---

## イベントカード

:card[1526]（限定ガチャ）と:card[1529]（総合ランキング）は注目のペアです。合わせると:card[1526]に+60%クリ率、65%クリダメ（累積）、60%最終ウェーブダメージが付与されます。ワールドボスメタに食い込むかはまだ分かりませんが、十分あり得ます。いずれにせよ全コンテンツで非常に強力なコンボです。:card[1526]は単体の1ラウンドダメージカードとしてすでに:card[1401]を超える可能性があります。ただし:card[1401]のように条件なしではなく2枠を必要とします。なお、:card[1529]のみが:card[1526]にボーナスを付与します——:card[228]は適用されません。今イベントの主要ターゲットです。**高優先度ペア——このイベント最大の目玉。**

:card[1528]（ランキング報酬 - 前半・後半）はDivinaチームのサポートまたはリザーブとしてそこそこ優秀です。スキルの影響は小さいですが、パッシブは堅実——Divina全員に25%攻撃速度、Divinaチームメンバーに35%最終ウェーブダメージ。**Divinaサポートとして優秀、スキルは弱め。**

:card[1531]（3月交換所）は:card[715]の小さな強化版で効果が2%増加しています。カードとして特別際立つほどではありませんが、入手しやすければ一部のチームコンプで使える可能性はあります。**ニッチ——入手しやすければ取っておこう。**

:card[1527]（スタンダードガチャ）は4★ながら希少性をはるかに上回る性能——攻撃力上位2体への44%クリダメは4★としては破格で、:card[600]に匹敵します。まだクリダメサポートが揃っていない新規プレイヤーにとって強力なリザーブ選択肢です。**新規プレイヤーへの強力なリザーブ——入手できれば取ろう。**

:card[1539]（スペシャルストーリー報酬）は極めて高耐久のAnima近接カードです。40%ダメージ軽減はほとんどのコンテンツの要求を超えていますが、どうしてもタンクが必要な場面では現在最強クラスの選択肢の一つです。HPが10%以下になると、永続50%ダメージブーストと5秒かけてHP10%回復を獲得——ピンチになるほど強くなります。**生存性がボトルネックになるコンテンツでの強力なニッチピック。**

:card[1540]（限定ガチャ）は:card[1494]の代替となる10%累積クリ率カードです。エンドレスやリザーブでは:card[1494]より使いやすく、:card[1494]はEXPブーストがあるためEXPモードでは依然として上です。:card[1494]がレベル75に達していない場合はターゲットにする価値があります。**Divinaリザーブとして優秀、特にエンドレス向け。**

:card[1590]（スタンダードガチャ——イベント後）はDivina上位2体のダメージをブーストしながら最終ウェーブで大きなダメージ軽減も提供する防御的なサポートカードです。リザーブでは:card[672]に近いですが、より防御寄り——高ダメージを求めるなら:card[672]が上です。**生存重視のDivinaリザーブとして堅実。ダメージを求めるなら:card[672]。**

---

## まとめ

- :card[1526]（限定ガチャ）+ :card[1529]（総合ランキング）——**高優先度ペア** - コンボ発動時のトップAnima単ラウンドダメージ、WBメタ候補
- :card[1528]（ランキング - 前半・後半）——**Divinaサポートとして優秀** - 堅実なパッシブ、スキルは弱め
- :card[1540]（限定ガチャ）——**Divinaリザーブとして優秀** - エンドレスでは:card[1494]より上
- :card[1539]（スペシャルストーリー報酬）——**強力なAnimaタンク** - 生存性が必要な場合の40%DR
- :card[1590]（スタンダードガチャ——イベント後）——**堅実なDivinaリザーブ** - WBとスペシャルストーリー/コンクエスト向け防御サポート
- :card[1527]（スタンダードガチャ）——**新規プレイヤー向け強力リザーブ** - 攻撃力上位2体への44%クリダメ、:card[600]に匹敵
- :card[1531]（3月交換所）——**ニッチ** - :card[715]の小アップグレード、入手しやすければ取ろう
- :card[1584]（スタンダードガチャ）——**スキップ** - EXP/ドロップヒーラー、:card[1321]の代替にはならない

---

*おすすめについて：好きなように遊んでください！欲しいカードがあれば取りに行きましょう。私のおすすめはコストを考慮しつつ、しばらく代替されにくい注目カードを挙げています。ゲームは比較的寛大なので、最強でないカードを取っても次のイベント用のリソースは残ります。好きなカードを買うのを躊躇わないでください！*

</div>

<div data-lang="ko">

옥션 소란이 지나고 신규 이벤트 카드 분석입니다.

---

## 이벤트 전 가챠

:card[1584]는 경험치와 드롭률을 올려주는 힐러로 일반 풀에 추가되었습니다. 대부분의 컨텐츠에서 :card[1321]을 대체한다고 생각하지 않습니다——스킬이 더 약하고, 디비나에는 부스트할 데미지 주문이 그다지 많지 않습니다. 5장이 생기면 쓸 수 있지만, 뽑을 가치는 없습니다. **스킵.**

---

## 이벤트 카드

:card[1526] *(한정 가챠)*과 :card[1529] *(종합 랭킹)*은 주목해야 할 쌍입니다. 함께 :card[1526]에 +60% 치명 확률, 65% 치명 피해(누적), 60% 최종 웨이브 데미지를 부여합니다. 월드 보스 메타에 진입할지는 아직 모르지만 가능성이 있습니다. 어떻든 콤보는 모든 컨텐츠에서 매우 강력합니다. :card[1526]은 이미 단일 라운드 데미지 카드로 :card[1401]을 능가할 가능성이 높지만, :card[1401]처럼 2칸을 차지하는 조건이 없습니다. :card[1529]만이 :card[1526]에 보너스를 부여하며 :card[228]은 적용되지 않습니다. 이 둘이 이번 이벤트 대부분 플레이어의 주요 목표가 될 것입니다. **높은 우선순위 쌍——이번 이벤트 최고의 카드.**

:card[1528] *(랭킹 보상 - 전반·후반)*은 디비나 팀의 서포트 또는 리저브로 꽤 강력합니다. 스킬의 영향은 크지 않지만 패시브는 탄탄합니다——디비나 아군 전체 25% 공격 속도, 디비나 팀원에게 35% 최종 웨이브 데미지. **좋은 디비나 서포트, 스킬은 약함.**

:card[1531] *(3월 교환소)*은 :card[715]의 소폭 업그레이드로 효과가 2% 증가합니다. 카드를 특별히 주목받게 만들기에는 부족하지만, 쉽게 접근 가능하다면 일부 팀 구성에서 쓸 수 있습니다. **틈새——쉽게 얻을 수 있다면 챙기자.**

:card[1527] *(일반 가챠)*은 4★이지만 희귀도를 훨씬 뛰어넘는 성능——공격력 상위 2아군에게 44% 치명 피해는 4★치고는 많은 양으로 :card[600]에 필적합니다. 아직 더 나은 치명 피해 서포트가 없는 신규 플레이어에게 강력한 리저브 선택지입니다. **신규 플레이어에게 강력한 리저브——접근 가능하면 챙기자.**

:card[1539] *(스페셜 스토리 보상)*은 극도로 단단한 아니마 근접 카드입니다. 40% 데미지 감소는 대부분의 컨텐츠가 요구하는 것보다 많지만, 어디서든 단단한 탱크가 필요할 때 현재 최고의 선택지 중 하나입니다. HP가 10% 이하로 떨어지면 영구 50% 데미지 부스트와 5초에 걸쳐 HP 10% 회복을 얻습니다——위기에 몰릴수록 강해집니다. **생존성이 병목인 컨텐츠에서 강력한 틈새 픽.**

:card[1540] *(한정 가챠)*은 :card[1494]의 대안인 10% 누적 치명 확률 카드입니다. 엔드리스나 리저브에서 더 유용하고, :card[1494]는 경험치 부스트 덕에 경험치 모드에서 여전히 우위입니다. :card[1494]가 레벨 75에 없다면 목표로 삼을 만합니다. **좋은 디비나 리저브, 특히 엔드리스용.**

:card[1590] *(일반 가챠——이벤트 후)*은 디비나 상위 2명의 데미지를 높이면서 최종 웨이브에 큰 데미지 감소도 제공하는 괜찮은 방어적 서포트입니다. 리저브에서 :card[672]와 비슷하지만 더 방어적——더 높은 데미지를 원한다면 :card[672]가 상위 선택입니다. **생존 중심 빌드에 탄탄한 디비나 리저브. 더 많은 데미지를 원한다면 :card[672].**

---

## 요약

- :card[1526] (한정 가챠) + :card[1529] (종합 랭킹) - **높은 우선순위 쌍** - 콤보 활성화 시 최고 단일 라운드 아니마 데미지, WB 메타 후보
- :card[1528] (랭킹 - 전반·후반) - **좋은 디비나 서포트** - 탄탄한 패시브, 스킬 약함
- :card[1540] (한정 가챠) - **좋은 디비나 리저브** - 엔드리스에서 :card[1494]보다 나음
- :card[1539] (스페셜 스토리 보상) - **강력한 아니마 탱크** - 생존성이 필요할 때 40% DR
- :card[1590] (일반 가챠——이벤트 후) - **탄탄한 디비나 리저브** - WB와 스페셜 스토리/컨퀘스트용 방어 서포트
- :card[1527] (일반 가챠) - **신규 플레이어에게 강력한 리저브** - 공격력 상위 2아군에게 44% 치명 피해, :card[600]에 필적
- :card[1531] (3월 교환소) - **틈새** - :card[715]의 소폭 업그레이드, 쉽게 얻을 수 있다면 챙기자
- :card[1584] (일반 가챠) - **스킵** - 경험치/드롭 힐러, :card[1321] 대체 불가

---

*추천에 대한 참고 사항: 원하는 대로 플레이하세요! 카드가 갖고 싶다면 구해보세요. 저의 추천은 비용을 고려하면서 당분간 대체될 가능성이 낮은 뛰어난 카드들을 강조합니다. 게임이 꽤 관대한 편이라 그다지 강하지 않은 카드를 구해도 다음 이벤트를 위한 자원을 확보할 수 있습니다. 좋아하는 카드를 구매하는 것을 망설이지 마세요!*

</div>

<div data-lang="zh-cn">

拍卖风波结束后，来看看新活动卡牌的分析。

---

## 活动前卡池

:card[1584]是一张提升经验值和掉落率的治疗者，现已加入常规池。我认为她在大多数内容中无法取代:card[1321]——技能较差，而且Divina本身能提升的伤害法术也不多。如果凑巧有5张可以用，但不值得专门抽取。**跳过。**

---

## 活动卡牌

:card[1526]（限定卡池）与:card[1529]（综合排名）是需要重点关注的组合。两者合计为:card[1526]提供+60%暴击率、65%暴击伤害（渐进）和60%最终波伤害。目前还不确定是否会打入世界BOSS元，但有可能。无论如何，这个组合在所有内容中都非常强力。:card[1526]的单轮伤害可能已经超越:card[1401]，不过:card[1401]没有需要两个位置的前提条件。注意只有:card[1529]能为:card[1526]提供加成——:card[228]不适用。这两张将是大多数玩家本次活动的主要目标。**高优先级组合——本次活动最值得入手的卡牌。**

:card[1528]（排名奖励——前半段·后半段）作为Divina队伍的辅助或备用相当强力。技能影响不大，但被动很扎实——Divina全员25%攻击速度，Divina队伍成员35%最终波伤害。**Divina辅助良好，技能较弱。**

:card[1531]（3月交换所）是:card[715]的小幅升级，效果提升2%。不足以让这张卡特别突出，但如果容易获取，在某些阵容中可以使用。**小众——容易获取则顺便拿下。**

:card[1527]（常规卡池）是4★但远超其稀有度——对攻击力前2名友军给予44%暴击伤害，对4★来说相当多，可与:card[600]相比。对于尚未拥有更好暴击伤害辅助的新手玩家来说是强力的备用选择。**新手玩家的强力备用——能获取就拿下。**

:card[1539]（特别剧情奖励）是极度坚韧的Anima近战卡。40%减伤超过大多数内容的需求，但在任何需要坚实坦克的情况下都是现有的最佳选择之一。当HP降至10%或以下时，永久获得50%伤害提升并在5秒内恢复10% HP——越危险越强力。**在生存能力成为瓶颈的内容中是强力的小众选择。**

:card[1540]（限定卡池）是:card[1494]的替代选择，提供10%渐进暴击率。在无尽或备用中可能更好用，而:card[1494]在经验模式中因为经验加成仍有优势。如果:card[1494]还未到75级，可以作为目标。**Divina备用的良好选择，尤其适合无尽模式。**

:card[1590]（常规卡池——活动后）是一张不错的防守辅助，为Divina上位2名提升伤害，同时在最终波提供大幅减伤。在备用位置与:card[672]相近但更偏防守——:card[672]是Divina备用位置中更高伤害的选择。**适合注重生存的Divina备用；想要更多伤害则选:card[672]。**

---

## 总结

- :card[1526]（限定卡池）+ :card[1529]（综合排名）——**高优先级组合** - 组合开启时顶级单轮Anima伤害，世界BOSS元候选
- :card[1528]（排名——前半段·后半段）——**Divina辅助良好** - 扎实的被动，技能较弱
- :card[1540]（限定卡池）——**Divina备用良好** - 无尽模式中优于:card[1494]
- :card[1539]（特别剧情奖励）——**强力Anima坦克** - 需要生存能力时的40%减伤
- :card[1590]（常规卡池——活动后）——**Divina备用扎实** - 世界BOSS及特别剧情/征服的防守辅助
- :card[1527]（常规卡池）——**新手玩家的强力备用** - 攻击力前2名44%暴击伤害，媲美:card[600]
- :card[1531]（3月交换所）——**小众** - :card[715]小幅升级，容易获取则顺便拿
- :card[1584]（常规卡池）——**跳过** - 经验/掉落治疗者，无法取代:card[1321]

---

*关于推荐的说明：按照自己喜欢的方式游玩！如果想要某张卡，就去获得它。我的推荐着重于一段时间内不太可能被取代的突出卡牌，同时兼顾成本。游戏相当慷慨，即使获得不那么强的卡牌，也能为下一个活动保留资源。不要犹豫购买你喜欢的卡牌！*

</div>

<div data-lang="zh-tw">

拍賣風波結束後，來看看新活動卡牌的分析。

---

## 活動前卡池

:card[1584]是一張提升經驗值和掉落率的治療者，現已加入常規池。我認為她在大多數內容中無法取代:card[1321]——技能較差，而且Divina本身能提升的傷害法術也不多。如果湊巧有5張可以用，但不值得專門抽取。**跳過。**

---

## 活動卡牌

:card[1526]（限定卡池）與:card[1529]（綜合排名）是需要重點關注的組合。兩者合計為:card[1526]提供+60%暴擊率、65%暴擊傷害（漸進）和60%最終波傷害。目前還不確定是否會打入世界BOSS元，但有可能。無論如何，這個組合在所有內容中都非常強力。注意只有:card[1529]能為:card[1526]提供加成——:card[228]不適用。這兩張將是大多數玩家本次活動的主要目標。**高優先級組合——本次活動最值得入手的卡牌。**

:card[1528]（排名獎勵——前半段·後半段）作為Divina隊伍的輔助或備用相當強力。技能影響不大，但被動很紮實——Divina全員25%攻擊速度，Divina隊伍成員35%最終波傷害。**Divina輔助良好，技能較弱。**

:card[1531]（3月交換所）是:card[715]的小幅升級，效果提升2%。不足以讓這張卡特別突出，但如果容易獲取，在某些陣容中可以使用。**小眾——容易獲取則順便拿下。**

:card[1527]（常規卡池）是4★但遠超其稀有度——對攻擊力前2名友軍給予44%暴擊傷害，對4★來說相當多，可與:card[600]相比。對於尚未擁有更好暴擊傷害輔助的新手玩家來說是強力的備用選擇。**新手玩家的強力備用——能獲取就拿下。**

:card[1539]（特別劇情獎勵）是極度堅韌的Anima近戰卡。40%減傷超過大多數內容的需求，但在任何需要堅實坦克的情況下都是現有的最佳選擇之一。當HP降至10%或以下時，永久獲得50%傷害提升並在5秒內恢復10% HP——越危險越強力。**在生存能力成為瓶頸的內容中是強力的小眾選擇。**

:card[1540]（限定卡池）是:card[1494]的替代選擇，提供10%漸進暴擊率。在無盡或備用中可能更好用，而:card[1494]在經驗模式中因為經驗加成仍有優勢。如果:card[1494]還未到75級，可以作為目標。**Divina備用的良好選擇，尤其適合無盡模式。**

:card[1590]（常規卡池——活動後）是一張不錯的防守輔助，為Divina上位2名提升傷害，同時在最終波提供大幅減傷。在備用位置與:card[672]相近但更偏防守——:card[672]是Divina備用位置中更高傷害的選擇。**適合注重生存的Divina備用；想要更多傷害則選:card[672]。**

---

## 總結

- :card[1526]（限定卡池）+ :card[1529]（綜合排名）——**高優先級組合** - 組合開啟時頂級單輪Anima傷害，世界BOSS元候選
- :card[1528]（排名——前半段·後半段）——**Divina輔助良好** - 紮實的被動，技能較弱
- :card[1540]（限定卡池）——**Divina備用良好** - 無盡模式中優於:card[1494]
- :card[1539]（特別劇情獎勵）——**強力Anima坦克** - 需要生存能力時的40%減傷
- :card[1590]（常規卡池——活動後）——**Divina備用紮實** - 世界BOSS及特別劇情/征服的防守輔助
- :card[1527]（常規卡池）——**新手玩家的強力備用** - 攻擊力前2名44%暴擊傷害，媲美:card[600]
- :card[1531]（3月交換所）——**小眾** - :card[715]小幅升級，容易獲取則順便拿
- :card[1584]（常規卡池）——**跳過** - 經驗/掉落治療者，無法取代:card[1321]

---

*關於推薦的說明：按照自己喜歡的方式遊玩！如果想要某張卡，就去獲得它。我的推薦著重於一段時間內不太可能被取代的突出卡牌，同時兼顧成本。遊戲相當慷慨，即使獲得不那麼強的卡牌，也能為下一個活動保留資源。不要猶豫購買你喜歡的卡牌！*

</div>

<div data-lang="es">

Después de toda la locura de las subastas, aquí está el análisis de las nuevas cartas del evento.

---

## Gacha Pre-Evento

:card[1584] es una sanadora que aumenta XP y la tasa de drops, ahora en el pool estándar. No creo que desplace a :card[1321] en la mayoría del contenido——su habilidad es peor y Divina no tiene tantos hechizos dañinos para potenciar. Es usable si te encuentras con 5 copias, pero probablemente no vale la pena gastar orbes. **Omitir.**

---

## Cartas del Evento

:card[1526] *(Gacha Limitada)* y :card[1529] *(Ranking General)* son el par a vigilar. Juntas dan a :card[1526] +60% de tasa crítica, 65% de daño crítico (progresivo) y 60% de daño en la oleada final. Aún no sé si esto penetrará en el meta del Jefe Mundial——es posible pero puede que tengamos que esperar y ver. En cualquier caso, el combo es muy bueno en todo el contenido. :card[1526] probablemente ya derroca a :card[1401] como la carta de mayor daño en una sola ronda, aunque :card[1401] no tiene la condición que requiere dos slots de carta. Nota que solo :card[1529] otorga el bonus a :card[1526]——:card[228] no se aplica. Estas dos serán los principales objetivos para la mayoría de los jugadores en este evento. **Par de alta prioridad——lo mejor que este evento tiene para ofrecer.**

:card[1528] *(Recompensa de Ranking - 1ª y 2ª Mitad)* es una selección de soporte o reserva bastante fuerte para equipos de Divina. Su habilidad no tiene mucho impacto, pero sus pasivos son sólidos——25% de velocidad de ataque para todos los aliados de Divina y 35% de daño en la oleada final para los miembros del equipo de Divina. **Buen soporte de Divina, habilidad débil.**

:card[1531] *(Intercambio de Marzo)* es una pequeña mejora a :card[715], aumentando el efecto en un 2%. No es suficiente para hacer que la carta sea especialmente relevante, pero probablemente sea usable en algunas composiciones de equipo si es fácilmente accesible. **De nicho——consíguela si es fácil de obtener.**

:card[1527] *(Gacha Estándar)* es de 4★ pero rinde muy por encima de su rareza——44% de Daño Crítico a los 2 aliados con el ATK más alto es mucho para un 4★, comparable a :card[600]. Una fuerte opción de reserva para jugadores más nuevos que aún no tienen mejores soportes de daño crítico. **Fuerte reserva para jugadores nuevos——consíguela si es accesible.**

:card[1539] *(Recompensa de Historia Especial)* es un cuerpo a cuerpo de Anima extremadamente resistente. Una reducción de daño del 40% es más de lo que la mayoría del contenido requiere, pero si te encuentras con alguna situación donde necesitas un tanque completamente sólido, esta es una de las mejores opciones disponibles. Cuando su HP cae al 10% o menos, gana un aumento de daño permanente del 50% y cura un 10% de HP en 5 segundos——cuanto más al límite está, más fuerte golpea. **Fuerte elección de nicho para cualquier contenido donde la supervivencia sea el cuello de botella.**

:card[1540] *(Gacha Limitada)* es una alternativa de 10% de tasa crítica progresiva a :card[1494]. Probablemente es mejor en Endless o como reserva, mientras que :card[1494] sigue siendo superior en los modos de XP gracias al aumento de XP. Si no tienes :card[1494] al nivel 75, este es uno a considerar. **Buena opción de reserva de Divina, especialmente para Endless.**

:card[1590] *(Gacha Estándar - post evento)* es un soporte defensivo decente que aumenta el daño de los dos principales aliados de Divina, mientras también proporciona una gran reducción de daño en la oleada final. Es comparable a :card[672] en reserva, pero más defensiva——:card[672] es la opción de mayor daño para los slots de reserva de Divina. **Sólida reserva de Divina para builds enfocadas en supervivencia; :card[672] si quieres más daño.**

---

## TL;DR

- :card[1526] (Gacha Limitada) + :card[1529] (Ranking General) - **Par de alta prioridad** - mayor daño de Anima en una sola ronda con el combo activo, posible meta de JM
- :card[1528] (Ranking - 1ª y 2ª Mitad) - **Buen soporte de Divina** - pasivo sólido, habilidad débil
- :card[1540] (Gacha Limitada) - **Buena reserva de Divina** - mejor que :card[1494] en Endless
- :card[1539] (Recompensa de Historia Especial) - **Fuerte tanque de Anima** - 40% RD si necesitas supervivencia
- :card[1590] (Gacha Estándar - post evento) - **Sólida reserva de Divina** - soporte defensivo para JM e Historia Especial/Conquista
- :card[1527] (Gacha Estándar) - **Fuerte reserva para jugadores nuevos** - 44% Daño Crítico a los 2 aliados con mayor ATK, comparable a :card[600]
- :card[1531] (Intercambio de Marzo) - **De nicho** - pequeña mejora a :card[715], consíguela si es accesible
- :card[1584] (Gacha Estándar) - **Omitir** - sanadora de XP/drops, no desplaza a :card[1321]

---

*Una nota sobre las recomendaciones: ¡Juega como quieras! Si quieres una carta, ve a por ella. Mis recomendaciones destacan las cartas que veo como sobresalientes——las que es poco probable que sean reemplazadas por un tiempo——siendo consciente del costo. El juego es bastante generoso, así que puedes conseguir cartas que no sean súper fuertes y aún tener recursos para el próximo evento. ¡No dudes en comprar una carta que te guste!*

</div>
