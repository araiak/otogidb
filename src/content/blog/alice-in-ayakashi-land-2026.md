---
title: "Alice in Ayakashi Land — New Event Cards"
description: "Card analysis for the Alice in Ayakashi Land event, covering the new event gacha, Conquest, and standard gacha cards"
date: "2026-03-27"
author: "Araiak"
tags: ["analysis", "gacha"]
---

<div data-lang="en">

Here's a first look at the new cards for Alice in Ayakashi Land. This is a quieter event compared to Elegance, which gave us :card[1526] and :card[1529] — both strong additions. Most of the cards here are situational or filler, with :card[1578] being the only one that looks like it could rise to that level.

---

## Limited Jewel Summons

:card[1578] is a Phantasma Alice — unusual given Alice is Divina, whether by design or an error. Most players will be eyeing her for her abilities in world boss. Her first ability is a 2-target 45% damage boost similar to :card[672], which is great. Her second ability is a 10% time extension. Time extend is an interesting one — in effect it gives you roughly 10% more time to attack, which translates to a roughly 10% damage increase assuming your team is alive in a single wave. There might be some edge cases around additional skill casts at the boundary. Where these really shine is with :card[514] due to her bug, which makes damage increase over time — in that case you get more than 10% because of the ramping damage. It could also have a similar effect in multi-wave battles with progressive skills, and given that recent events have had shorter timers, the extra seconds may result in more reliable Conquest clears. Compared to other time extension options she is considerably stronger than :card[209] or :card[267]. Her skill is a single-target version of :card[739]'s skill, so she's usable in lineup but probably best in reserve. **Very worth getting and probably the top target for this event.**

*Update:* LJS as expected, with the featured banner running April 5–9.

*Update (April 5):* A last-minute change slipped through with the event launch — :card[1578] herself was changed from **Phantasma to Divina**, and her Classic Idol ability was updated alongside it to restrict to **Divina allies only**. The description now reads "2 divina allies with the highest ATK," and the data confirms both. The original analysis noted the Phantasma read as unusual; turns out it was corrected before launch. Whether that happened early in development or at the last minute, we can't say for certain.

If you already pulled, don't panic. The immediate value takes a hit — she's a poor fit outside Divina teams — but she's still a strong card in the right context. Divina's top carry has arguably been :card[553] for a long time now, and a high-crit-rate dedicated damage carry is overdue. If something like that lands in the next cycle or two, :card[1578] slots right back in as a premier support. **If you're a Divina player, this doesn't change much — she's still very good for you. If you're not, don't overinvest, but grab the exchange copy — there's a decent chance she becomes valuable down the line, whether that's a Divina team that wants her or an auction flip when she eventually hits the market.**

:card[1638] looks like a standard gacha option to revisit after the event. It has a little defense and a 25% shield debuff on all foes in the final wave. This could stack with :card[642] or :card[661] for considerable shield debuff on teams that want more consistent damage or lack debuffers like :card[1311]. Values on these skills are a bit low and they require fairly high investment — the cost may be too high to justify for most players. **If you find yourself with copies it could be worth using if you need a mix of defense and shield debuff-based damage boost; not worth targeting.**

*Update:* Confirmed LJS — featured banner April 9–12, with exchange. Not the priority pick of this event, but if you're looking for shield debuff coverage, this is your window.

---

## Conquest Cards

:card[1576] is a better version of :card[1435] — 3-target progressive skill damage with additional damage reduction. That might be ok in challenge mode but probably not your first choice. **Grab it if it's a Conquest card; not worth pushing for if it ends up as a ranking reward.**

*Update:* Conquest card, so go ahead and grab her.

:card[1577] is in the mold of :card[1489] but provides a [wild] melee slot for Anima. A lot of people are interested in seeing what they come up with for the card model here, but otherwise it isn't particularly special. :card[1539] is probably more reliably tanky with a worse skill in most modes. Of the [wild] cards, this is probably the strongest given it has damage reduction, attack speed, and HP restoration. It could also be a decent tank for newer Anima players if you can reliably trigger the ability — though ideally you'd want more support abilities from a tank. **If it's a Conquest card, grab an MLB; it's probably not worth pushing for otherwise.**

*Update:* Conquest card confirmed — grab the MLB. Attack speed came in at 20% rather than 40%, which puts him more in line with the other [wild] melee cards, but doesn't change the rating much.

---

## Ranking Cards

:card[1579] looks like a potential ranking card overall. She gives 10% progressive crit damage to Divina and 25% attack speed. Divina has good access to attack speed and she may allow teams to more easily hit attack speed caps. Progressive crit damage is ok in multi-wave content — 50% crit damage in 5 waves is solid for a card that targets the whole team. Her skill is also notable: 71% crit rate lands on the cards that are going to do the most damage, especially in Divina where self crit rate cards are few and far between.

The tricky part is how she fits into an established Divina lineup. For newer players or teams without other crit rate sources, she's excellent — her skill pushes damage up significantly, and 71% crit rate to your top two damage dealers genuinely moves the needle. For teams that already have crit rate coverage, she can still be effective: hitting 100% crit on your top two damage dealers is genuinely strong. The consideration is that cards like :card[1494] or :card[1540], with their passive crit scaling over multiple waves, can provide more total team crit with less overcapping — so in rosters that already include them, you may get more mileage from a card that contributes damage in a different way. But pushing her to reserve isn't a great answer either — her progressive crit damage ability is decent, but it's not a standout reserve contribution compared to something like :card[672] or :card[1590]. She is also a ranged ATK bond, which is a good reason to pick up at least a copy or two even if you don't see yourself using her in lineup. **Good and usable for a lot of rosters, but not top tier — worth getting if accessible, not worth breaking the bank for.**

:spoiler["Her attack speed is currently bugged — data suggests 10% not 25%. This would be a considerable loss making her much harder to slot into teams where she might justify a space today."]

:card[1581] also looks like a potential event LJS card. Her 30% critical damage looks decent but is probably beat out by :card[1992] — 30% damage is better in any situation where your damage isn't already extremely high. The damage buff on cast is too low to be strong. Her skill might have niche use as a cleanse, but so far there hasn't been much need for a cleanse in most content, though it's probably nice for some world bosses. **Not a priority — there are better options for the slot in most teams.**

*Update:* She turned out to be a ranking reward rather than LJS, which is actually the better outcome — you pick up copies just for participating, though it's not worth PvP orbing to MLB for the crit damage even once the bug is fixed. Players who top rankings won't need her, but she's a nice freebie for the rest of the field. When she eventually hits auction the interesting comparison is :card[655] — in most situations :card[655] is still the stronger pick, but if that crit skill would overcap your main carry, :card[1581] is worth considering instead, not by a lot. She's a better fit on passive damage teams than on teams that do burst damage during a crit buff window. **Grab your ranking copies and move on.**

:spoiler["She is currently bugged to only be 5% crit damage, making her a non-starter. It's probably safe to assume this will be fixed."]

---

## Standard Gacha

:card[1458] may be LJS. He has a greedy 15% crit damage and crit rate per wave buff and a 3-target damage skill — nearly a copy of :card[1314]. I still don't see a great place for greedy progressive cards in most formats. They're fine in Conquest, Special Story, and challenge, but if you can find a team buff that's usually stronger than a greedy AoE attacker. Where you really benefit is from a greedy single-target attacker, and diluting skill damage reduces your clear speed outside challenge. The ATK bond for ranged might make it worth grabbing a copy if he ends up on exchange and you don't already have double Phantasma ATK ranged bonds. **Worth a copy if you need the ranged ATK bond; otherwise situational.**

*Update:* He's standard gacha with a featured banner April 12–19, not a limited card as I had assumed. Since he'll be in the permanent pool you'll pick up copies over time — the ATK bond is still a decent reason to have a copy, but there's no rush.

:card[1636] — cut-off art notwithstanding — is probably too situational to justify investment. It's in the standard gacha, so if you happen to end up with 4 copies it could be useful for adjusting critical rate from reserve. But 18% crit for Anima is probably not enough to bump other strong reserve options. If you're running :card[1526] and :card[1529], you'd get more crit by splitting off a copy of :card[1529] for the same reserve slot. If :card[1526] falls out of the meta then maybe :card[1636] could serve a similar role — crit rate is often very strong compared to other stats percent-for-percent, even when crit rate values are already fairly high. **Situational; only worth slotting if you happen to hit 4 copies and need Anima crit rate coverage.**

---

## Monthly Exchange

:card[1582] looks like a 4★ or an exchange card — probably not a buy for anyone but collectors. **Pass unless you're collecting.**

*Update:* Monthly exchange as expected — nothing changes here.

---

## TL;DR

Limited Jewel Summons:
- :card[1578] — **Divina players: still a top target**. Classic Idol was updated to Divina-only at launch — grab the exchange copy regardless, but don't overinvest if you're not a Divina player. Long-term value holds if a Divina carry arrives soon, and one is arguably overdue
- :card[1638] — **Niche**. Shield debuff stacking with :card[642] or :card[661]; limited so now's the window if you want it

Conquest Cards:
- :card[1577] — **Grab the MLB**. [Wild] Anima melee with damage reduction, attack speed, and HP restore; decent tank for newer Anima players
- :card[1576] — **Grab it**. Better version of :card[1435] with damage reduction added

Ranking Cards:
- :card[1579] — **Worth getting**, not worth breaking the bank. Progressive crit damage for Divina plus attack speed and a ranged ATK bond; best for players who can slot her into lineup. *See bug note.*
- :card[1581] — **Grab your ranking copies and move on**. Bugged for now — once fixed, a decent option on passive damage teams or anywhere :card[655]'s crit skill would overcap your carry

Standard Gacha:
- :card[1458] — **Situational**. Not urgent since he's in the permanent pool, but worth a copy if you need the ranged ATK bond
- :card[1636] — **Situational**. Only worth slotting if you happen to hit 4 copies and need Anima crit rate coverage

Monthly Exchange:
- :card[1582] — **Collectors only**

---

*A note on recommendations: Play how you like! If you want a card, go for it. My recommendations highlight cards I see as standouts while being mindful of cost. The game is fairly generous, so you can pick up cards that aren't super strong and still have resources for the next event. Don't hesitate to buy a card you like!*

</div>

<div data-lang="ja">

アリス・イン・アヤカシランドの新カードをひとつひとつ見ていきましょう。Eleganceイベントで:card[1526]と:card[1529]という強力なカードが登場した後、今回は比較的おとなしいイベントです。全体的にはニッチまたは補填向けのカードが多く、:card[1578]だけが同レベルに迫る可能性を感じさせます。

---

## 限定ジュエルサモン

:card[1578]はPhantasmaのアリスです——アリスはDivinaのはずなので、意図的なのかエラーなのか気になるところです。多くのプレイヤーはワールドボスでのアビリティに注目しているでしょう。1つ目のアビリティは:card[672]に似た2体への45%ダメージブーストで、非常に優秀です。2つ目のアビリティは10%の時間延長。時間延長は面白い効果で——実質的に攻撃時間が10%増えることになり、チームが1ラウンド生き残れる前提であれば約10%のダメージ増加に相当します。境界付近でのスキル追加発動など、エッジケースもあるかもしれません。特に効果的なのは:card[514]との組み合わせで、彼女のバグによりダメージが時間経過とともに増加するため、時間延長で得られる恩恵が10%以上になります。累積スキルを使うマルチラウンドの戦闘でも同様の効果が期待でき、最近のイベントではタイマーが短い傾向にある中、追加の数秒がコンクエスト攻略の安定につながる場合もあります。他の時間延長オプション（:card[209]や:card[267]）と比べてもかなり強力です。スキルは:card[739]のシングルターゲット版なので、ラインナップ入りも可能ですが、おそらくリザーブがベストです。**非常に価値があり、今イベントの最優先ターゲット。**

*Update:* 予想通りLJS——フィーチャーバナーは4月5日〜9日。

:card[1638]はイベント後に改めて見直したいスタンダードガチャの選択肢のように見えました。少量の防御と、最終ラウンドの全敵への25%シールドデバフを持ちます。:card[642]や:card[661]との重ねがけで、安定したダメージが欲しいチームや:card[1311]のようなデバッファーを持たないチームには有効なシールドデバフが作れます。スキルの数値はやや低く、必要投資量も多め——コストに見合わない場合もあります。**コピーが手元にあり防御とシールドデバフを組み合わせた構成が必要なら使えるかもしれない；積極的には狙わなくていい。**

*Update:* LJS確定——フィーチャーバナーは4月9日〜12日、交換所あり。今イベントの最優先ではないですが、シールドデバフが必要なら今がチャンスです。

---

## コンクエストカード

:card[1576]は:card[1435]の上位版——3体への累積スキルダメージにダメージ軽減が追加されています。チャレンジモードでは活躍できるかもしれませんが、最初の選択肢にはならないでしょう。**コンクエストカードなら取っておく価値あり；ランキング報酬なら無理して狙う必要はなし。**

*Update:* コンクエストカード確定——そのまま取りに行こう。

:card[1577]は:card[1489]に近いカードですが、Animaに[ワイルド]近接スロットを提供します。カードモデルへの期待は高いようですが、性能面では特別際立った点はありません。:card[1539]の方がほとんどのモードで安定して耐久が高く、スキルは劣ります。[ワイルド]カードの中では、ダメージ軽減・攻撃速度・HP回復を備えたこのカードがおそらく最も優秀です。アビリティを確実に発動できるなら、Anima初心者にとって悪くないタンクになるでしょう——ただし理想的にはタンクにはもっとサポート系アビリティが欲しいところです。**コンクエストカードならMLBを目指そう；そうでなければ無理して取りに行く必要はなし。**

*Update:* コンクエストカード確定——MLBを取りに行こう。攻撃速度は40%ではなく20%でした。他の[ワイルド]近接カードと同水準で、総合評価への影響は大きくありません。

---

## ランキングカード

:card[1579]はランキング報酬の有力候補です。Divina全体に10%の累積クリダメと25%の攻撃速度を付与します。Divinaは攻撃速度へのアクセスが豊富で、このカードでキャップに届きやすくなるかもしれません。マルチラウンドのコンテンツでは累積クリダメが活きてきます——5ラウンドで50%クリダメはチーム全体に効くカードとしてしっかりした数値です。スキルも注目で、71%クリ率をダメージに最も貢献するカードに付与します。Divinaはセルフクリ率カードが少ないため、特に有効です。

既存のDivinaラインナップへの組み込み方が難しいところです。クリ率源がまだ少ない新規・中堅プレイヤーにとっては非常に優秀——スキルがダメージを大きく底上げし、71%クリ率をトップ2のダメージディーラーに乗せる効果は確かに大きいです。すでにクリ率を確保しているチームでも有効で、トップ2を100%クリに乗せることは純粋に強いです。ただし:card[1494]や:card[1540]のような、マルチラウンドで累積するクリ率カードを既に持っているなら、無駄なオーバーキャップを避けながら別の形でダメージを伸ばせるカードの方が効果的かもしれません。とはいえリザーブに回すのも最適解ではなく——累積クリダメアビリティは及第点ですが、:card[672]や:card[1590]のようなリザーブの強カードと比べると見劣りします。遠距離ATKボンドを持つことも評価ポイントで、ラインナップに入れなくても1〜2枚確保する理由になります。**多くのロースターで使えるカードですが、トップ性能ではない——入手しやすければ取る価値あり、無理して狙う必要はなし。**

:spoiler["攻撃速度は現在バグの可能性があります——データでは25%ではなく10%を示しています。これが正しければかなりの損失で、現時点で採用を検討しているチームでは組み込みが難しくなります。"]

:card[1581]は限定LJSカードの候補かと思っていました。30%クリダメはそこそこ良さそうですが、:card[1992]に軍配が上がるでしょう——ダメージがそもそも極端に高くない状況なら30%ダメージの方が優秀です。スキルのダメージバフも効果が低すぎて強みになりません。スキルのクレンズは一部ワールドボスでニッチな使い道があるかもしれませんが、今のところほとんどのコンテンツではクレンズの必要性はあまりありません。**優先度低——ほとんどのチームでより良い選択肢がある。**

*Update:* LJSではなくランキング報酬でした——これは実際には良い結果で、参加するだけでコピーが集まります。ただし、バグが修正されても、クリダメのためにPvPオーブを使ってMLBにするほどの価値はありません。ランキング上位陣には不要なカードですが、それ以外のプレイヤーには嬉しい無料報酬です。いずれオークションに出た際の比較相手は:card[655]——大半の状況では:card[655]の方が上ですが、あのクリスキルがメインキャリーをオーバーキャップするような場面では:card[1581]が選択肢になります、大きな差ではありませんが。パッシブダメージチームとの相性は良く、クリバフ窓でのバーストダメージに依存するチームには向きません。**ランキングコピーを確保して次へ進もう。**

:spoiler["現在クリダメが5%にバグっており、実戦投入は難しい状況です。修正が入るものと思われます。"]

---

## スタンダードガチャ

:card[1458]はLJSかもしれないと思っていました。1ラウンドごとに15%のクリダメとクリ率が溜まる累積バフと、3体対象のダメージスキルを持ち——:card[1314]にほぼ近い性能です。累積プログレッシブカードには多くのフォーマットで入れやすい場所がないと感じています。コンクエスト・スペシャルストーリー・チャレンジでは使えますが、チームバフが使えるならそちらの方が大抵上です。真に恩恵を受けるのは累積シングルターゲットアタッカーからで、スキルダメージが分散するAoEはチャレンジ以外ではクリア速度を落とします。遠距離ATKボンドが、交換所に出た際に1枚確保する理由にはなりますが、Phantasma遠距離のATKボンドが2つ揃っていない場合に限ります。**ATKボンドが必要なら1枚の価値あり；そうでなければ状況次第。**

*Update:* 最初はLJSだと思っていましたが、4月12日〜19日にフィーチャーバナーが来るスタンダードガチャカードでした。永続プールに入るので時間をかけてコピーが集まります——ATKボンドのために1枚持っておく理由はありますが、急ぐ必要はありません。

:card[1636]は——カットオフしたアートはともかく——投資を正当化するには状況が限定的すぎるでしょう。スタンダードガチャなので4枚揃えばリザーブからクリ率を調整するのに使えるかもしれません。ただし18%のAnimaクリ率では他の強いリザーブオプションを押しのけるには不十分です。:card[1526]と:card[1529]を使っているなら、同じリザーブ枠で:card[1529]を1枚分けた方がクリが高くなります。:card[1526]がメタから外れた場合は:card[1636]が似た役割を担えるかもしれません——クリ率がすでに高い状態でも他のステータス比で非常に優秀なことが多いので。**状況次第；4枚揃ってAnimaクリ率の補充が必要な場合のみ採用を検討。**

---

## 月替わり交換所

:card[1582]は4★か交換カードのような見た目——コレクター以外に買う理由はなさそうです。**コレクター以外はパス。**

*Update:* 予想通り月替わり交換所カード——評価に変わりなし。

---

## まとめ

限定ジュエルサモン:
- :card[1578] — **今イベントの最優先ターゲット**。45%の2体ダメージブーストと10%時間延長；リザーブでの運用がベスト、:card[514]との相性が特に良くワールドボスで高い価値を発揮
- :card[1638] — **ニッチ**。:card[642]や:card[661]とのシールドデバフ重ねに使えるかも；限定なので欲しければ今がチャンス

コンクエストカード:
- :card[1577] — **MLBを取りに行こう**。ダメージ軽減・攻撃速度・HP回復を持つ[ワイルド]Anima近接；Anima初心者にとって悪くないタンク
- :card[1576] — **取っておこう**。ダメージ軽減付きの:card[1435]上位版

ランキングカード:
- :card[1579] — **取る価値あり**、無理する必要はなし。Divina向け累積クリダメ＋攻撃速度と遠距離ATKボンド；ラインナップに入れられるプレイヤーに特に有効。*バグ情報参照。*
- :card[1581] — **ランキングコピーを確保して次へ**。現在バグ中——修正後はパッシブダメージチームや:card[655]のクリスキルがキャリーをオーバーキャップする場面でそれなりの選択肢に

スタンダードガチャ:
- :card[1458] — **状況次第**。永続プール入りなので急がなくていいが、遠距離ATKボンドが必要なら1枚持っておく価値あり
- :card[1636] — **状況次第**。4枚揃ってAnimaクリ率の補充が必要な場合のみ採用検討
- :card[1638] — **ニッチ**。:card[642]や:card[661]とのシールドデバフ重ねに使えるかも；積極的には狙わなくていい

月替わり交換所:
- :card[1582] — **コレクター以外はパス**

---

*おすすめについて：好きなように遊んでください！欲しいカードがあれば取りに行きましょう。私のおすすめはコストを考慮しつつ、目立った活躍が見込めるカードを挙げています。ゲームは比較的寛大なので、最強でないカードを取っても次のイベント用のリソースは残ります。好きなカードを買うのを躊躇わないでください！*

</div>

<div data-lang="ko">

옥션 소란이 지나고 아리스 인 아야카시 랜드의 신규 카드 분석입니다. Elegance 이벤트에서 :card[1526]과 :card[1529]처럼 강력한 카드들이 등장했지만, 이번 이벤트는 비교적 조용한 편입니다. 대부분의 카드는 상황에 따라 다르거나 보조 역할에 그치며, :card[1578]만이 그 수준에 가까울 수 있는 카드로 보입니다.

---

## 한정 가챠

:card[1578]은 Phantasma의 Alice입니다——Alice는 Divina인데 의도적인지 오류인지 궁금합니다. 대부분의 플레이어는 월드 보스에서의 어빌리티에 주목할 것입니다. 첫 번째 어빌리티는 :card[672]와 유사한 2체 45% 데미지 부스트로 매우 우수합니다. 두 번째 어빌리티는 10% 시간 연장입니다. 시간 연장은 흥미로운 효과로——실질적으로 공격 시간이 10% 늘어나며, 팀이 1라운드 동안 살아있다고 가정하면 약 10%의 데미지 증가에 해당합니다. 경계 부근에서의 스킬 추가 발동 같은 엣지 케이스도 있을 수 있습니다. 특히 효과적인 것은 :card[514]와의 조합으로, 그녀의 버그로 인해 데미지가 시간이 지날수록 증가하기 때문에 시간 연장으로 10% 이상의 효과를 얻을 수 있습니다. 누적 스킬을 사용하는 멀티라운드 전투에서도 비슷한 효과가 기대되며, 최근 이벤트의 타이머가 짧아진 추세에서 추가 시간이 컨퀘스트 공략 안정성을 높일 수 있습니다. 다른 시간 연장 옵션(:card[209]나 :card[267])과 비교해도 훨씬 강력합니다. 스킬은 :card[739]의 싱글 타겟 버전이므로 라인업에도 사용 가능하지만, 리저브가 베스트입니다. **매우 가치 있으며, 이번 이벤트 최우선 목표.**

*Update:* 예상대로 LJS——피처드 배너는 4월 5일〜9일.

:card[1638]은 이벤트 후에 다시 살펴볼 일반 가챠 선택지처럼 보였습니다. 소량의 방어력과 최종 라운드 전 적에게 25% 실드 디버프를 지닙니다. :card[642]나 :card[661]과 중첩하여 안정적인 데미지를 원하는 팀이나 :card[1311] 같은 디버퍼가 없는 팀에 상당한 실드 디버프를 만들 수 있습니다. 스킬 수치가 다소 낮고 필요 투자량도 많아——코스트에 맞지 않는 경우가 있습니다. **코피가 있고 방어와 실드 디버프 조합이 필요하다면 쓸 수 있을지도；적극적으로 노릴 가치는 없음.**

*Update:* LJS 확정——피처드 배너 4월 9〜12일, 교환소 포함. 이번 이벤트 최우선 픽은 아니지만, 실드 디버프가 필요하다면 지금이 기회입니다.

---

## 컨퀘스트 카드

:card[1576]은 :card[1435]의 상위 버전——3체 누적 스킬 데미지에 데미지 감소가 추가되었습니다. 챌린지 모드에서는 괜찮을 수 있지만 첫 번째 선택지는 아닐 것입니다. **컨퀘스트 카드라면 챙겨두자；랭킹 보상이라면 무리할 필요 없음.**

*Update:* 컨퀘스트 카드 확정——챙겨 가자.

:card[1577]은 :card[1489]와 유사하지만 Anima에게 [와일드] 근접 슬롯을 제공합니다. 카드 모델에 대한 기대가 높은 것 같지만, 성능 면에서는 특별히 두드러진 점이 없습니다. :card[1539]가 대부분의 모드에서 더 안정적인 탱킹 성능을 보입니다. [와일드] 카드 중에서는 데미지 감소·공격 속도·HP 회복을 갖춘 이 카드가 가장 강력합니다. 어빌리티를 안정적으로 발동할 수 있다면 Anima 초보자에게 괜찮은 탱크가 될 수 있습니다——다만 이상적으로는 탱크에게 더 많은 서포트 어빌리티가 있으면 좋겠습니다. **컨퀘스트 카드라면 MLB를 목표로；그렇지 않다면 무리할 필요 없음.**

*Update:* 컨퀘스트 카드 확정——MLB를 챙기자. 공격 속도는 40%가 아닌 20%였습니다. 다른 [와일드] 근접 카드들과 비슷한 수준이며 전체 평가에 큰 영향은 없습니다.

---

## 랭킹 카드

:card[1579]는 랭킹 보상 카드로 보입니다. Divina 전체에 10% 누적 크리 데미지와 25% 공격 속도를 부여합니다. Divina는 공격 속도 수단이 풍부하며 이 카드로 캡에 더 쉽게 도달할 수 있을 것입니다. 누적 크리 데미지는 멀티라운드 컨텐츠에서 활약합니다——5라운드에 50% 크리 데미지는 팀 전체에 효과가 있는 카드로서 탄탄한 수치입니다. 스킬도 주목할 만한데, 데미지에 가장 크게 기여하는 카드에 71% 치명 확률을 부여합니다. Divina는 셀프 크리 확률 카드가 부족한 편이라 특히 유효합니다.

기존 Divina 라인업에 어떻게 맞추느냐가 까다로운 점입니다. 크리 확률 수단이 아직 부족한 신규·중급 플레이어에게는 매우 우수합니다——스킬이 데미지를 크게 끌어올리며, 상위 2명의 딜러에게 71% 크리 확률을 부여하는 효과는 확실히 체감됩니다. 이미 크리 확률을 확보한 팀에서도 효과적일 수 있습니다——상위 2명을 100% 크리로 맞추는 것은 순수하게 강합니다. 단, :card[1494]나 :card[1540]처럼 멀티라운드에서 누적되는 크리 확률 카드를 이미 보유하고 있다면, 불필요한 오버캡 없이 다른 방식으로 데미지를 높이는 카드가 더 효과적일 수 있습니다. 리저브로 보내는 것도 최선이 아닌데——누적 크리 데미지 어빌리티는 그럭저럭이지만, :card[672]나 :card[1590] 같은 강력한 리저브 카드에 비해 뒤떨어집니다. 원거리 ATK 본드도 평가 요소로, 라인업에 넣지 않더라도 1〜2장 확보할 이유가 됩니다. **많은 로스터에서 사용 가능하지만 최고 성능은 아님——접근 가능하면 가져가자, 무리할 필요는 없음.**

:spoiler["공격 속도에 현재 버그가 있을 수 있습니다——데이터는 25%가 아닌 10%를 보여줍니다. 맞다면 상당한 손실로, 현재 채용을 고려하고 있는 팀에서 조합이 어려워집니다."]

:card[1581]은 이벤트 LJS 카드 후보로 생각했습니다. 30% 크리 데미지는 그럭저럭 좋아 보이지만 :card[1992]에 밀립니다——데미지가 극단적으로 높지 않은 상황에서는 30% 데미지가 더 우수합니다. 발동 시 데미지 버프도 효과가 너무 낮아 강점이 되지 않습니다. 스킬의 클렌즈는 일부 월드 보스에서 틈새 용도가 있을 수 있지만, 지금까지 대부분의 컨텐츠에서 클렌즈의 필요성은 크지 않았습니다. **우선순위 낮음——대부분의 팀에서 더 나은 선택지가 있음.**

*Update:* LJS가 아닌 랭킹 보상이었습니다——이는 실제로 더 좋은 결과로, 참가하기만 해도 코피가 모입니다. 다만 버그가 수정되더라도 크리 데미지를 위해 PvP 오브로 MLB를 맞출 가치는 없습니다. 상위 랭커에게는 필요 없는 카드지만 그 외 플레이어에게는 반가운 무료 보상입니다. 언젠가 옥션에 나왔을 때의 비교 대상은 :card[655]——대부분의 상황에서 :card[655]가 더 강하지만, 그 크리 스킬이 메인 캐리를 오버캡할 경우 :card[1581]이 선택지가 됩니다, 큰 차이는 아니지만. 패시브 데미지 팀과의 궁합이 좋고, 크리 버프 윈도우에서 버스트 데미지를 노리는 팀에는 맞지 않습니다. **랭킹 코피를 확보하고 넘어가자.**

:spoiler["현재 크리 데미지가 5%로 버그 상태여서 실전 투입이 어렵습니다. 수정이 들어올 것으로 생각됩니다."]

---

## 일반 가챠

:card[1458]은 LJS일 것으로 생각했습니다. 1라운드마다 15%의 크리 데미지와 크리 확률이 쌓이는 누적 버프와 3체 대상 데미지 스킬을 보유해——:card[1314]와 거의 동일한 성능입니다. 누적 프로그레시브 카드는 대부분의 포맷에서 넣기 좋은 자리를 찾기 어렵습니다. 컨퀘스트·스페셜 스토리·챌린지에서는 쓸 만하지만, 팀 버프가 있다면 대개 그쪽이 더 우수합니다. 진정한 효과를 내는 것은 누적 싱글 타겟 어태커이며, AoE로 스킬 데미지를 분산시키면 챌린지 이외에서는 클리어 속도가 떨어집니다. 원거리 ATK 본드가 교환소에 나왔을 때 1장 확보하는 이유가 될 수 있지만, Phantasma 원거리 ATK 본드가 이미 2개 갖춰지지 않은 경우에 한합니다. **ATK 본드가 필요하다면 1장의 가치 있음；그렇지 않다면 상황에 따라.**

*Update:* LJS로 생각했지만 4월 12〜19일에 피처드 배너가 있는 일반 가챠 카드였습니다. 영구 풀에 들어가므로 시간이 지나면 자연스럽게 코피가 모입니다——ATK 본드를 위해 1장 갖고 있을 이유는 있지만 서두를 필요는 없습니다.

:card[1636]은——아트의 잘린 부분은 제쳐두고——투자를 정당화하기에는 상황이 너무 제한적입니다. 일반 가챠이므로 4장이 모이면 리저브에서 크리 확률을 조절하는 데 쓸 수 있습니다. 하지만 18%의 Anima 크리 확률은 다른 강력한 리저브 옵션을 밀어낼 만큼 충분하지 않습니다. :card[1526]과 :card[1529]를 운용 중이라면, 같은 리저브 슬롯에서 :card[1529]를 1장 분리하는 것이 더 많은 크리를 얻는 방법입니다. :card[1526]이 메타에서 벗어난다면 :card[1636]이 비슷한 역할을 맡을 수 있습니다——크리 확률은 이미 높은 상태에서도 다른 스탯 대비 비율로 매우 강한 경우가 많으므로. **상황에 따라；4장이 모여 Anima 크리 확률 보충이 필요한 경우에만 채용 검토.**

---

## 월간 교환소

:card[1582]는 4★이나 교환소 카드처럼 보입니다——콜렉터 이외에는 구매 이유가 없을 것 같습니다. **콜렉터 아니라면 패스.**

*Update:* 예상대로 월간 교환소 카드——평가 변화 없음.

---

## 요약

한정 가챠:
- :card[1578] — **이번 이벤트 최우선 목표**. 45% 2체 데미지 부스트와 10% 시간 연장；리저브가 베스트, :card[514]와의 시너지로 월드 보스에서 높은 가치 발휘
- :card[1638] — **틈새**. :card[642]나 :card[661]과 실드 디버프 중첩에 쓸 수 있음；한정이니 원한다면 지금이 기회

컨퀘스트 카드:
- :card[1577] — **MLB를 챙기자**. 데미지 감소·공격 속도·HP 회복을 가진 [와일드] Anima 근접；Anima 초보자에게 나쁘지 않은 탱크
- :card[1576] — **챙겨 가자**. 데미지 감소가 추가된 :card[1435] 상위 버전

랭킹 카드:
- :card[1579] — **가져갈 가치 있음**, 무리할 필요는 없음. Divina 누적 크리 데미지＋공격 속도와 원거리 ATK 본드；라인업에 넣을 수 있는 플레이어에게 특히 효과적. *버그 정보 참조.*
- :card[1581] — **랭킹 코피 확보하고 넘어가자**. 현재 버그 상태——수정 후 패시브 데미지 팀이나 :card[655]의 크리 스킬이 캐리를 오버캡할 경우 그럭저럭인 선택지로

일반 가챠:
- :card[1458] — **상황에 따라**. 영구 풀에 있어 서두를 필요 없지만, 원거리 ATK 본드가 필요하다면 1장 가치 있음
- :card[1636] — **상황에 따라**. 4장이 모여 Anima 크리 확률 보충이 필요한 경우만 채용 검토

월간 교환소:
- :card[1582] — **콜렉터 아니라면 패스**

---

*추천에 대한 참고 사항: 원하는 대로 플레이하세요! 카드가 갖고 싶다면 구해보세요. 저의 추천은 비용을 고려하면서 눈에 띄는 활약이 기대되는 카드들을 강조합니다. 게임이 꽤 관대한 편이라 그다지 강하지 않은 카드를 구해도 다음 이벤트를 위한 자원을 확보할 수 있습니다. 좋아하는 카드를 구매하는 것을 망설이지 마세요!*

</div>

<div data-lang="zh-cn">

拍卖风波结束后，来看看爱丽丝异界仙境的新活动卡牌分析。与给我们带来:card[1526]和:card[1529]的Elegance活动相比，这次活动相对平静。大多数卡牌定位小众或作为补充，只有:card[1578]看起来有可能达到那个水平。

---

## 限定卡池

:card[1578]是Phantasma属性的Alice——考虑到Alice本是Divina，不知是有意为之还是出错了。大多数玩家会关注她在世界BOSS中的能力。第一个被动是类似:card[672]的2体45%伤害提升，非常出色。第二个被动是10%时间延长。时间延长是个有趣的效果——实际上给你多了约10%的攻击时间，假设队伍在单轮中存活，大约相当于10%的伤害提升。在边界处可能存在额外技能触发的边缘情况。特别有效的是与:card[514]的搭配，由于她的bug使伤害随时间增长，时间延长带来的收益超过10%。在使用渐进技能的多轮战斗中也可能有类似效果，考虑到近期活动计时器较短，额外几秒可能有助于更稳定地通关塔。与其他时间延长选项（:card[209]或:card[267]）相比强得多。技能是:card[739]技能的单体版，可以上阵，但最好放在备用位置。**非常值得获取，很可能是本次活动的首要目标。**

*Update:* 如预期的LJS——精选卡池运行时间为4月5日至9日。

:card[1638]看起来像活动后值得重新审视的常规卡池选择。有少量防御，并对最终轮所有敌人施加25%实盾减益。可以与:card[642]或:card[661]叠加，对想要更稳定伤害或缺乏:card[1311]这类减益卡的队伍形成可观的实盾减益。技能数值偏低且需要较高投入——成本可能难以为多数玩家所接受。**手上有副本且需要防御和实盾减益组合的话可以用；不值得主动去追。**

*Update:* 确认为LJS——精选卡池4月9日至12日，附带交换所。不是本次活动的首要目标，但如果需要实盾减益覆盖，现在是机会。

---

## 征服卡

:card[1576]是:card[1435]的升级版——3体渐进技能伤害附带额外减伤。在挑战模式中可能不错，但可能不是首选。**是征服卡就值得拿；如果是排名奖励就没必要强求了。**

*Update:* 确认为征服卡——直接拿下吧。

:card[1577]与:card[1489]相似，但为Anima提供了[万能]近战位。很多人对这张卡的模型很感兴趣，不过性能上并没有特别出彩的地方。:card[1539]在大多数模式中坦性更稳，但技能较弱。在[万能]卡中，这张拥有减伤、攻击速度和HP回复，可能是最强的。如果能稳定触发被动，对Anima新手来说是不错的坦克——不过理想情况下坦克应该有更多辅助被动。**是征服卡就冲MLB；否则没必要强求。**

*Update:* 确认为征服卡——拿下MLB。攻击速度实为20%而非40%，与其他[万能]近战卡相近，对整体评价影响不大。

---

## 排名卡

:card[1579]看起来是排名奖励卡。她为Divina全员提供10%渐进暴击伤害和25%攻击速度。Divina获取攻击速度的途径较多，这张卡可能有助于更轻松地达到上限。渐进暴击伤害在多轮内容中发挥作用——5轮后50%暴击伤害对全体队伍生效，对于这类卡来说是相当扎实的数值。技能也值得关注：71%暴击率给到伤害贡献最大的卡牌，在Divina中自身暴击率卡本来就少，特别有效。

如何融入现有的Divina阵容是个难题。对于暴击率来源还不足的新手和中级玩家来说非常优秀——技能能显著提升伤害，对顶级2名输出给予71%暴击率实实在在地改变战局。对于已经有暴击率覆盖的队伍也有效：让顶级2名达到100%暴击率本身就很强。不过，如果已有:card[1494]或:card[1540]这类多轮渐进暴击率卡，可能还是用不会造成无效超额的方式补充伤害更合适。推到备用也不是最优解——渐进暴击伤害被动还行，但与:card[672]或:card[1590]这样的强力备用卡相比就差一些了。远程ATK羁绊也是个加分项，即使不上阵也有理由拿1到2张。**很多阵容都能用，但不是顶级——容易获取就拿，不值得大力投资。**

:spoiler["攻击速度可能存在bug——数据显示是10%而非25%。如果属实将是相当大的损失，让今天想把她纳入阵容的队伍更难安排。"]

:card[1581]原本以为是活动LJS卡。30%暴击伤害看起来还不错，但可能比不上:card[1992]——在伤害没有极端之高的情况下，30%伤害更优秀。释放时的伤害加成效果太低，不成为优势。技能的净化在某些世界BOSS中可能有小众用途，但迄今为止大多数内容对净化的需求并不高。**优先度低——大多数阵容有更好的选择。**

*Update:* 是排名奖励而非LJS——实际上是更好的结果，仅靠参与就能积累副本。不过即使bug修复后，也不值得消耗PvP水晶将她打到MLB。排名靠前的玩家并不需要她，但对其他玩家来说是个不错的免费奖励。等她上拍卖时有意思的比较对象是:card[655]——大多数情况下:card[655]仍更强，但如果那个暴击技能会让主要输出超额，:card[1581]值得考虑，差距不大。被动伤害队与她更搭，依赖在暴击增益窗口内爆发伤害的队伍与她不太合适。**拿下排名副本继续前进。**

:spoiler["目前暴击伤害有5%的bug，难以实战运用。应该会有修复。"]

---

## 常规卡池

:card[1458]原本以为是LJS。他拥有每轮积累15%暴击伤害和暴击率的渐进buff和3体伤害技能——与:card[1314]几乎相同。渐进累积卡在大多数玩法中很难找到合适位置。在征服、特别剧情和挑战中可以使用，但如果能用队伍buff通常更强。真正受益的是渐进单体攻击者，而AoE分散技能伤害会降低挑战模式以外的通关速度。远程ATK羁绊可以成为在交换所出现时入手一张的理由，前提是Phantasma远程ATK羁绊还没有凑齐两个。**需要ATK羁绊就值得一张；否则看情况。**

*Update:* 原以为是LJS，但实际上是4月12日至19日有精选卡池的常规卡池卡。因为会进入永久池，慢慢会积累副本——ATK羁绊仍是入手一张的理由，但不用着急。

:card[1636]——撇开裁剪的插图不谈——可能太小众了，难以支撑投入。它在常规卡池中，所以凑巧有4张的话可以用于从备用调整暴击率。不过18%的Anima暴击率可能不足以取代其他强力备用选项。如果你在用:card[1526]和:card[1529]，拆分一张:card[1529]到同一备用位可以获得更多暴击率。如果:card[1526]退出元游，:card[1636]或许能承担类似角色——即使在暴击率已较高时，与其他属性相比按比例来看暴击率通常非常强。**视情况而定；只有凑满4张且需要Anima暴击率覆盖时才考虑上阵。**

---

## 月度交换所

:card[1582]看起来像4★或交换所卡——除了收藏玩家外，大概没有购买理由。**收藏以外请路过。**

*Update:* 如预期的月度交换所卡——评价不变。

---

## 总结

限定卡池:
- :card[1578] — **本次活动首要目标**。45%双体伤害提升和10%时间延长；备用位最佳，与:card[514]配合在世界BOSS中价值极高
- :card[1638] — **小众**。可与:card[642]或:card[661]叠加实盾减益；限定卡，想要的话现在是机会

征服卡:
- :card[1577] — **冲MLB**。拥有减伤、攻击速度和HP回复的[万能]Anima近战；对Anima新手还不错的坦克
- :card[1576] — **拿下**。添加减伤的:card[1435]升级版

排名卡:
- :card[1579] — **值得入手**，不必大力投资。Divina渐进暴击伤害＋攻击速度和远程ATK羁绊；能上阵的玩家效果最佳。*请见bug说明。*
- :card[1581] — **拿下排名副本继续前进**。目前bug状态——修复后在被动伤害队或:card[655]暴击技能会让输出超额的情况下是还不错的选择

常规卡池:
- :card[1458] — **视情况而定**。进了永久池不用着急，但需要远程ATK羁绊的话值得入手一张
- :card[1636] — **视情况而定**。只有凑满4张且需要Anima暴击率覆盖时才考虑

月度交换所:
- :card[1582] — **收藏以外请路过**

---

*关于推荐的说明：按照自己喜欢的方式游玩！如果想要某张卡，就去获得它。我的推荐着重于一段时间内不太可能被取代的出色卡牌，同时兼顾成本。游戏相当慷慨，即使获得不那么强的卡牌，也能为下一个活动保留资源。不要犹豫购买你喜欢的卡牌！*

</div>

<div data-lang="zh-tw">

拍賣風波結束後，來看看愛麗絲異界仙境的新活動卡牌分析。與給我們帶來:card[1526]和:card[1529]的Elegance活動相比，這次活動相對平靜。大多數卡牌定位小眾或作為補充，只有:card[1578]看起來有可能達到那個水平。

---

## 限定卡池

:card[1578]是Phantasma屬性的Alice——考慮到Alice本是Divina，不知是有意為之還是出錯了。大多數玩家會關注她在世界BOSS中的能力。第一個被動是類似:card[672]的2體45%傷害提升，非常出色。第二個被動是10%時間延長。時間延長是個有趣的效果——實際上給你多了約10%的攻擊時間，假設隊伍在單輪中存活，大約相當於10%的傷害提升。在邊界處可能存在額外技能觸發的邊緣情況。特別有效的是與:card[514]的搭配，由於她的bug使傷害隨時間增長，時間延長帶來的收益超過10%。在使用漸進技能的多輪戰鬥中也可能有類似效果，考慮到近期活動計時器較短，額外幾秒可能有助於更穩定地通關塔。與其他時間延長選項（:card[209]或:card[267]）相比強得多。技能是:card[739]技能的單體版，可以上陣，但最好放在備用位置。**非常值得獲取，很可能是本次活動的首要目標。**

*Update:* 如預期的LJS——精選卡池運行時間為4月5日至9日。

:card[1638]看起來像活動後值得重新審視的常規卡池選擇。有少量防禦，並對最終輪所有敵人施加25%實盾減益。可以與:card[642]或:card[661]疊加，對想要更穩定傷害或缺乏:card[1311]這類減益卡的隊伍形成可觀的實盾減益。技能數值偏低且需要較高投入——成本可能難以為多數玩家所接受。**手上有副本且需要防禦和實盾減益組合的話可以用；不值得主動去追。**

*Update:* 確認為LJS——精選卡池4月9日至12日，附帶交換所。不是本次活動的首要目標，但如果需要實盾減益覆蓋，現在是機會。

---

## 征服卡

:card[1576]是:card[1435]的升級版——3體漸進技能傷害附帶額外減傷。在挑戰模式中可能不錯，但可能不是首選。**是征服卡就值得拿；如果是排名獎勵就沒必要強求了。**

*Update:* 確認為征服卡——直接拿下吧。

:card[1577]與:card[1489]相似，但為Anima提供了[萬能]近戰位。很多人對這張卡的模型很感興趣，不過性能上並沒有特別出彩的地方。:card[1539]在大多數模式中坦性更穩，但技能較弱。在[萬能]卡中，這張擁有減傷、攻擊速度和HP回復，可能是最強的。如果能穩定觸發被動，對Anima新手來說是不錯的坦克——不過理想情況下坦克應該有更多輔助被動。**是征服卡就衝MLB；否則沒必要強求。**

*Update:* 確認為征服卡——拿下MLB。攻擊速度實為20%而非40%，與其他[萬能]近戰卡相近，對整體評價影響不大。

---

## 排名卡

:card[1579]看起來是排名獎勵卡。她為Divina全員提供10%漸進暴擊傷害和25%攻擊速度。Divina獲取攻擊速度的途徑較多，這張卡可能有助於更輕鬆地達到上限。漸進暴擊傷害在多輪內容中發揮作用——5輪後50%暴擊傷害對全體隊伍生效，對於這類卡來說是相當紮實的數值。技能也值得關注：71%暴擊率給到傷害貢獻最大的卡牌，在Divina中自身暴擊率卡本來就少，特別有效。

如何融入現有的Divina陣容是個難題。對於暴擊率來源還不足的新手和中級玩家來說非常優秀——技能能顯著提升傷害，對頂級2名輸出給予71%暴擊率實實在在地改變戰局。對於已經有暴擊率覆蓋的隊伍也有效：讓頂級2名達到100%暴擊率本身就很強。不過，如果已有:card[1494]或:card[1540]這類多輪漸進暴擊率卡，可能還是用不會造成無效超額的方式補充傷害更合適。推到備用也不是最優解——漸進暴擊傷害被動還行，但與:card[672]或:card[1590]這樣的強力備用卡相比就差一些了。遠程ATK羈絆也是個加分項，即使不上陣也有理由拿1到2張。**很多陣容都能用，但不是頂級——容易獲取就拿，不值得大力投資。**

:spoiler["攻擊速度可能存在bug——數據顯示是10%而非25%。如果屬實將是相當大的損失，讓今天想把她納入陣容的隊伍更難安排。"]

:card[1581]原本以為是活動LJS卡。30%暴擊傷害看起來還不錯，但可能比不上:card[1992]——在傷害沒有極端之高的情況下，30%傷害更優秀。釋放時的傷害加成效果太低，不成為優勢。技能的淨化在某些世界BOSS中可能有小眾用途，但迄今為止大多數內容對淨化的需求並不高。**優先度低——大多數陣容有更好的選擇。**

*Update:* 是排名獎勵而非LJS——實際上是更好的結果，僅靠參與就能積累副本。不過即使bug修復後，也不值得消耗PvP水晶將她打到MLB。排名靠前的玩家並不需要她，但對其他玩家來說是個不錯的免費獎勵。等她上拍賣時有意思的比較對象是:card[655]——大多數情況下:card[655]仍更強，但如果那個暴擊技能會讓主要輸出超額，:card[1581]值得考慮，差距不大。被動傷害隊與她更搭，依賴在暴擊增益窗口內爆發傷害的隊伍與她不太合適。**拿下排名副本繼續前進。**

:spoiler["目前暴擊傷害有5%的bug，難以實戰運用。應該會有修復。"]

---

## 常規卡池

:card[1458]原本以為是LJS。他擁有每輪積累15%暴擊傷害和暴擊率的漸進buff和3體傷害技能——與:card[1314]幾乎相同。漸進累積卡在大多數玩法中很難找到合適位置。在征服、特別劇情和挑戰中可以使用，但如果能用隊伍buff通常更強。真正受益的是漸進單體攻擊者，而AoE分散技能傷害會降低挑戰模式以外的通關速度。遠程ATK羈絆可以成為在交換所出現時入手一張的理由，前提是Phantasma遠程ATK羈絆還沒有湊齊兩個。**需要ATK羈絆就值得一張；否則看情況。**

*Update:* 原以為是LJS，但實際上是4月12日至19日有精選卡池的常規卡池卡。因為會進入永久池，慢慢會積累副本——ATK羈絆仍是入手一張的理由，但不用著急。

:card[1636]——撇開裁剪的插圖不談——可能太小眾了，難以支撐投入。它在常規卡池中，所以湊巧有4張的話可以用於從備用調整暴擊率。不過18%的Anima暴擊率可能不足以取代其他強力備用選項。如果你在用:card[1526]和:card[1529]，拆分一張:card[1529]到同一備用位可以獲得更多暴擊率。如果:card[1526]退出元遊，:card[1636]或許能承擔類似角色——即使在暴擊率已較高時，與其他屬性相比按比例來看暴擊率通常非常強。**視情況而定；只有湊滿4張且需要Anima暴擊率覆蓋時才考慮上陣。**

---

## 月度交換所

:card[1582]看起來像4★或交換所卡——除了收藏玩家外，大概沒有購買理由。**收藏以外請路過。**

*Update:* 如預期的月度交換所卡——評價不變。

---

## 總結

限定卡池:
- :card[1578] — **本次活動首要目標**。45%雙體傷害提升和10%時間延長；備用位最佳，與:card[514]配合在世界BOSS中價值極高
- :card[1638] — **小眾**。可與:card[642]或:card[661]疊加實盾減益；限定卡，想要的話現在是機會

征服卡:
- :card[1577] — **衝MLB**。擁有減傷、攻擊速度和HP回復的[萬能]Anima近戰；對Anima新手還不錯的坦克
- :card[1576] — **拿下**。添加減傷的:card[1435]升級版

排名卡:
- :card[1579] — **值得入手**，不必大力投資。Divina漸進暴擊傷害＋攻擊速度和遠程ATK羈絆；能上陣的玩家效果最佳。*請見bug說明。*
- :card[1581] — **拿下排名副本繼續前進**。目前bug狀態——修復後在被動傷害隊或:card[655]暴擊技能會讓輸出超額的情況下是還不錯的選擇

常規卡池:
- :card[1458] — **視情況而定**。進了永久池不用著急，但需要遠程ATK羈絆的話值得入手一張
- :card[1636] — **視情況而定**。只有湊滿4張且需要Anima暴擊率覆蓋時才考慮

月度交換所:
- :card[1582] — **收藏以外請路過**

---

*關於推薦的說明：按照自己喜歡的方式遊玩！如果想要某張卡，就去獲得它。我的推薦著重於一段時間內不太可能被取代的出色卡牌，同時兼顧成本。遊戲相當慷慨，即使獲得不那麼強的卡牌，也能為下一個活動保留資源。不要猶豫購買你喜歡的卡牌！*

</div>

<div data-lang="es">

Después del caos de las subastas, aquí está el análisis de las nuevas cartas de Alice in Ayakashi Land. Es un evento más tranquilo comparado con Elegance, que nos dio :card[1526] y :card[1529]——ambas incorporaciones fuertes. La mayoría de las cartas aquí son situacionales o de relleno, siendo :card[1578] la única que parece poder alcanzar ese nivel.

---

## Gacha Limitada

:card[1578] es una Alice de Phantasma——inusual dado que Alice es Divina, ya sea por diseño o por error. La mayoría de los jugadores la mirarán por sus habilidades en el Jefe Mundial. Su primera habilidad es un aumento de daño del 45% a 2 objetivos similar a :card[672], que es genial. Su segunda habilidad es una extensión de tiempo del 10%. La extensión de tiempo es interesante——en efecto te da aproximadamente un 10% más de tiempo para atacar, lo que se traduce en un aumento de daño de aproximadamente el 10% asumiendo que tu equipo esté vivo en una sola ronda. Puede haber algunos casos extremos alrededor de lanzamientos de habilidades adicionales en el límite. Donde esto realmente brilla es con :card[514] debido a su bug, que hace que el daño aumente con el tiempo——en ese caso obtienes más del 10% debido al daño en aumento. También podría tener un efecto similar en batallas de múltiples rondas con habilidades progresivas, y dado que eventos recientes han tenido temporizadores más cortos, los segundos extra pueden resultar en clears de Conquista más confiables. Comparada con otras opciones de extensión de tiempo es considerablemente más fuerte que :card[209] o :card[267]. Su habilidad es una versión de un solo objetivo de la habilidad de :card[739], así que es usable en el equipo pero probablemente mejor en reserva. **Muy vale la pena obtener y probablemente el objetivo principal de este evento.**

*Update:* LJS como se esperaba——el banner destacado corre del 5 al 9 de abril.

:card[1638] parece una opción de gacha estándar para revisar después del evento. Tiene un poco de defensa y un 25% de debuff de escudo en todos los enemigos en la ronda final. Esto podría apilarse con :card[642] o :card[661] para un debuff de escudo considerable en equipos que quieran daño más consistente o carezcan de desbufadores como :card[1311]. Los valores en estas habilidades son un poco bajos y requieren una inversión bastante alta——el costo puede ser demasiado alto para la mayoría de los jugadores. **Si te encuentras con copias podría valer la pena usarla si necesitas una mezcla de defensa y debuff de escudo; no vale la pena apuntar a ella.**

*Update:* Confirmada como LJS——banner destacado del 9 al 12 de abril, con intercambio. No es el objetivo prioritario del evento, pero si buscas cobertura de debuff de escudo, esta es tu ventana.

---

## Cartas de Conquista

:card[1576] es una versión mejorada de :card[1435]——daño de habilidad progresivo a 3 objetivos con reducción de daño adicional. Podría estar bien en modo desafío pero probablemente no es tu primera opción. **Consíguela si es carta de Conquista; no vale la pena esforzarse si acaba siendo recompensa de ranking.**

*Update:* Carta de Conquista confirmada——adelante y consíguela.

:card[1577] está en el molde de :card[1489] pero proporciona un slot cuerpo a cuerpo [libre] para Anima. Mucha gente está interesada en ver qué hacen con el modelo de la carta, pero por lo demás no es particularmente especial. :card[1539] es probablemente más confiablemente resistente con una peor habilidad en la mayoría de los modos. De las cartas [libre], esta es probablemente la más fuerte dado que tiene reducción de daño, velocidad de ataque y restauración de HP. También podría ser un tanque decente para jugadores más nuevos de Anima si puedes activar la habilidad de manera confiable——aunque idealmente querrías más habilidades de soporte de un tanque. **Si es carta de Conquista, consigue el MLB; probablemente no vale la pena esforzarse de otra manera.**

*Update:* Carta de Conquista confirmada——consigue el MLB. La velocidad de ataque resultó ser 20% en lugar de 40%, más en línea con las otras cartas cuerpo a cuerpo [libre], pero no cambia mucho la valoración.

---

## Cartas de Ranking

:card[1579] parece una carta de ranking en general. Da 10% de daño crítico progresivo a Divina y 25% de velocidad de ataque. Divina tiene buen acceso a velocidad de ataque y puede permitir que los equipos alcancen los límites más fácilmente. El daño crítico progresivo está bien en contenido de múltiples rondas——50% de daño crítico en 5 rondas es sólido para una carta que afecta a todo el equipo. Su habilidad también es notable: 71% de tasa crítica aterriza en las cartas que van a hacer el mayor daño, especialmente en Divina donde las cartas de tasa crítica propia son pocas.

La parte complicada es cómo encaja en una formación de Divina establecida. Para jugadores más nuevos o equipos sin otras fuentes de tasa crítica, es excelente——su habilidad empuja el daño de manera significativa, y 71% de tasa crítica a tus 2 mejores repartidores de daño genuinamente mueve la aguja. Para equipos que ya tienen cobertura de tasa crítica, todavía puede ser efectiva: llegar al 100% de crítico en tus 2 mejores repartidores de daño es genuinamente fuerte. La consideración es que cartas como :card[1494] o :card[1540], con su escalado de tasa crítica pasivo a lo largo de múltiples rondas, pueden proporcionar más tasa crítica total del equipo con menos exceso——así que en rosters que ya las incluyen, puede que obtengas más provecho de una carta que contribuye daño de manera diferente. Pero empujarla a reserva tampoco es la mejor respuesta——su habilidad de daño crítico progresivo es decente, pero no es una contribución de reserva destacada comparada con algo como :card[672] o :card[1590]. También es un vínculo de ATK a distancia, lo que es una buena razón para conseguir al menos una o dos copias incluso si no la ves en tu formación. **Buena y usable para muchos rosters, pero no de primer nivel——vale la pena obtenerla si es accesible, no vale la pena gastar una fortuna.**

:spoiler["Su velocidad de ataque está actualmente bugeada——los datos sugieren 10% no 25%. Esto sería una pérdida considerable haciendo mucho más difícil incluirla en equipos donde podría justificar un espacio hoy."]

:card[1581] también parecía una posible carta LJS del evento. Su 30% de daño crítico parece decente pero probablemente lo supera :card[1992]——30% de daño es mejor en cualquier situación donde tu daño no sea ya extremadamente alto. El aumento de daño al lanzar es demasiado bajo para ser fuerte. Su habilidad podría tener uso nicho como limpieza, pero hasta ahora no ha habido mucha necesidad de limpieza en la mayoría del contenido, aunque probablemente sea útil para algunos jefes mundiales. **No es prioridad——hay mejores opciones para el slot en la mayoría de los equipos.**

*Update:* Resultó ser una recompensa de ranking en lugar de LJS——un mejor resultado de lo esperado, ya que acumulas copias solo por participar. No vale la pena gastar orbes de PvP para llevarla a MLB por el daño crítico incluso una vez solucionado el bug. Los jugadores que encabezan los rankings no la necesitan, pero es un buen regalo gratuito para el resto. Cuando finalmente llegue a la subasta, la comparación interesante es :card[655]——en la mayoría de situaciones :card[655] sigue siendo la opción más fuerte, pero si esa habilidad crítica sobrepasaría el límite de tu carry principal, :card[1581] vale la pena considerar, no por mucho. Encaja mejor en equipos de daño pasivo que en equipos que hacen daño explosivo durante ventanas de buff crítico. **Consigue tus copias de ranking y sigue adelante.**

:spoiler["Actualmente está bugeada a solo 5% de daño crítico, haciéndola un no-arranque. Probablemente es seguro asumir que esto se solucionará."]

---

## Gacha Estándar

:card[1458] podría ser LJS. Tiene un buff progresivo codicioso de 15% de daño crítico y tasa crítica por ronda y una habilidad de daño a 3 objetivos——casi una copia de :card[1314]. Todavía no veo un gran lugar para las cartas progresivas codiciosas en la mayoría de los formatos. Están bien en Conquista, Historia Especial y desafío, pero si puedes encontrar un buff de equipo eso es generalmente más fuerte que un atacante AoE codicioso. Donde realmente te beneficias es de un atacante de un solo objetivo codicioso, y diluir el daño de habilidad reduce tu velocidad de clear fuera del desafío. El vínculo de ATK a distancia podría hacer que valga la pena conseguir una copia si acaba en el intercambio y aún no tienes doble vínculo ATK a distancia de Phantasma. **Vale una copia si necesitas el vínculo ATK a distancia; de lo contrario situacional.**

*Update:* Es gacha estándar con un banner destacado del 12 al 19 de abril, no una carta limitada como había asumido. Como estará en el pool permanente irás acumulando copias con el tiempo——el vínculo ATK sigue siendo una razón para tener una copia, pero no hay prisa.

:card[1636]——el arte cortado aparte——probablemente es demasiado situacional para justificar inversión. Está en la gacha estándar, así que si acabas con 4 copias podría ser útil para ajustar la tasa crítica desde reserva. Pero 18% de crítico para Anima probablemente no es suficiente para superar otras opciones de reserva fuertes. Si estás usando :card[1526] y :card[1529], obtendrías más crítico dividiendo una copia de :card[1529] para el mismo slot de reserva. Si :card[1526] cae del meta entonces quizás :card[1636] podría servir un rol similar——la tasa crítica es a menudo muy fuerte comparada con otras estadísticas porcentualmente, incluso cuando los valores de tasa crítica ya son bastante altos. **Situacional; solo vale incluirla si acabas con 4 copias y necesitas cobertura de tasa crítica de Anima.**

---

## Intercambio Mensual

:card[1582] parece una carta de 4★ o intercambio——probablemente no es una compra para nadie excepto coleccionistas. **Pasa de largo a menos que colecciones.**

*Update:* Intercambio mensual como se esperaba——nada cambia aquí.

---

## TL;DR

Gacha Limitada:
- :card[1578] — **Objetivo principal de este evento**. Aumento de daño del 45% a 2 objetivos y extensión de tiempo del 10%; mejor en reserva, excelente valor en Jefe Mundial especialmente con :card[514]
- :card[1638] — **De nicho**. Apilado de debuff de escudo con :card[642] o :card[661]; limitada, así que ahora es tu oportunidad si la quieres

Cartas de Conquista:
- :card[1577] — **Consigue el MLB**. Cuerpo a cuerpo [libre] de Anima con reducción de daño, velocidad de ataque y restauración de HP; tanque decente para jugadores más nuevos de Anima
- :card[1576] — **Consíguela**. Versión mejorada de :card[1435] con reducción de daño añadida

Cartas de Ranking:
- :card[1579] — **Vale la pena obtener**, no vale la pena gastar una fortuna. Daño crítico progresivo de Divina más velocidad de ataque y vínculo ATK a distancia; mejor para jugadores que pueden incluirla en la formación. *Ver nota de bug.*
- :card[1581] — **Consigue tus copias de ranking y sigue adelante**. Bugeada por ahora——una vez solucionado, una opción decente en equipos de daño pasivo o donde la habilidad crítica de :card[655] sobrepasaría el límite de tu carry

Gacha Estándar:
- :card[1458] — **Situacional**. No hay prisa ya que está en el pool permanente, pero vale una copia si necesitas el vínculo ATK a distancia
- :card[1636] — **Situacional**. Solo considera incluirla si acabas con 4 copias y necesitas cobertura de tasa crítica de Anima

Intercambio Mensual:
- :card[1582] — **Solo para coleccionistas**

---

*Una nota sobre las recomendaciones: ¡Juega como quieras! Si quieres una carta, ve a por ella. Mis recomendaciones destacan las cartas que veo como sobresalientes mientras tengo en cuenta el costo. El juego es bastante generoso, así que puedes conseguir cartas que no sean súper fuertes y aún tener recursos para el próximo evento. ¡No dudes en comprar una carta que te guste!*

</div>
